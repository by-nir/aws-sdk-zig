...
pub const ServiceConfig = struct {
    ...
    fn fullname(comptime self: ServiceConfig) []const u8 {
        return if (self.access) |acc| self.name ++ "-" ++ acc else self.name;
    }
};
...
pub fn init(allocator: Allocator, comptime service: ServiceConfig, user: UserOptions) !Self {
...
pub fn isVirtualHostableS3Bucket(label: []const u8, allow_subdomains: bool) bool {
    if (!smithy.url.isValidHostLabel(label, allow_subdomains)) return false;
    if (allow_subdomains) {
