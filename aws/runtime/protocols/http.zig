...
pub fn uriMetaLabels(
    ...
    const fmt_str, const Args, const args_indices = comptime blk: {
        ...
        while (it.next()) |part| {
            ...
            for (labels, 0..) |meta, i| {
                ...
                const fmt_part, const arg_typ = switch (@as(smithy.SerialType, member.scheme.shape)) {
                    .boolean,
    ...
    inline for (0..fmt_args.len) |i| {
        ...
        fmt_args[i] = switch (@as(smithy.SerialType, member.scheme.shape)) {
            .int_enum => @intFromEnum(value),
            .byte, .short, .integer, .long => value,
            .boolean, .float, .double => try stringifyValue(allocator, member.scheme, value),
            .string,
            .str_enum,
            .trt_enum,
            .timestamp_date_time,
            .timestamp_http_date,
            .timestamp_epoch_seconds,
            => .{ .raw = try stringifyValue(allocator, member.scheme, value) },
            inline .big_integer, .big_decimal => |g| @compileError("Unimplemted label type " ++ @tagName(g)),
            else => unreachable,
        };
    }
    ...
    inline for (params) |meta| {
        ...
        switch (@as(smithy.MetaParam, meta[0])) {
                ...
                switch (@as(smithy.SerialType, scheme.shape)) {
                    inline .list_dense, .list_sparse => |g| {
                        ...
                        for (value) |val| {
                            ...
                            try values.append(try stringifyValue(allocator, scheme.member, val));
                        }
            ...
            .header_map => {
                ...
                const item_type: smithy.SerialType = scheme.shape;
                ...
                while (it.next()) |entry| {
                    ...
                    switch (@as(smithy.SerialType, scheme.shape)) {
                        ...
                        .list_dense => {
                            ...
                            const str_vals = if (scheme.member.shape == .string) items else blk: {
                        ...
                        .list_sparse => {
                            ...
                            for (items) |t| {
                                if (t == null) continue;
                                try str_vals.append(if (scheme.member.shape == .string) t else t.toString());
                            }
                        ...
                        .set => {
                            ...
                            while (set_it.next()) |s| {
                                try str_vals.append(if (scheme.member.shape == .string) s else s.toString());
                            }
    ...
    if (@hasField(@TypeOf(scheme), "transport")) {
        ...
        switch (@as(smithy.MetaTransport, meta[0])) {
            .status_code => {
                ...
                @field(value, member.name_zig) = switch (@as(smithy.SerialType, member.scheme.shape)) {
                        ...
                        switch (@as(smithy.SerialType, target.shape)) {
                            .list_dense, .list_sparse => {
                                ...
                                while (val_it.next()) |val| : (i += 1) {
                                    try parseStringValue(allocator, target.scheme, val, &list[i]);
                                }
                            },
                            .set => {
                                ...
                                while (val_it.next()) |val| {
                                    ...
                                    try parseStringValue(allocator, target.scheme, val, &item);
                    ...
                    if (mem.startsWith(u8, header.name, prefix)) {
                        ...
                        switch (srl.findMemberIndex(fields, name)) {
                            inline 0...(fields.len - 1) => |i| {
                                ...
                                try parseStringValue(allocator, field.scheme, header.value, target_value);
                                continue :headers;
                            },
                            else => unreachable,
                        }
...
fn stringifyValue(arena: Allocator, comptime scheme: anytype, value: anytype) ![]const u8 {
    switch (@as(srl.SerialType, scheme.shape)) {
        .string => return value,
