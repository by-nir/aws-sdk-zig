...
pub fn prefixed(self: Self, bytes: []const u8) !void {
...
pub fn line(self: Self, bytes: []const u8) !void {
...
pub fn list(self: Self, comptime T: type, items: []const T, comptime options: ListOptions) !void {
...
test "list" {
    ...
    try writer.list([]const u8, &.{ "foo", "bar", "baz" }, .{
        .item_format = "s",
    });
