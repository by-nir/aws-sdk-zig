...
const smithy_tasks = @import("smithy.zig");
const ScopeTag = smithy_tasks.ScopeTag;
...
pub const ServiceCodegenClient = smithy_tasks.ServiceScriptGen.Task("Smithy Service Client Codegen", serviceCodegenClientTask, .{
    .injects = &.{SymbolsProvider},
});
fn serviceCodegenClientTask(self: *const Delegate, symbols: *SymbolsProvider, bld: *ContainerBuild) anyerror!void {
    while (symbols.next()) |id| {
        try self.evaluate(WriteShape, .{ bld, id });
    }
}

test "ServiceCodegenClient" {
    ...
    var symbols = try test_symbols.setup(test_alloc, &.{.root_child});
    defer symbols.deinit();
    _ = try tester.provideService(&symbols, null);
    try symbols.enqueue(SmithyId.of("test#Root"));

    try smithy_tasks.expectServiceScript(
        \\pub const Root = []const Child;
        \\
        \\pub const Child = []const i32;
    , ServiceCodegenClient, tester.pipeline, .{});
}

const WriteShape = Task.Define("Smithy Write Shape", writeShapeTask, .{
...
fn writeShapeTask(
    ...
    const policy = self.readValue(CodegenPolicy, ScopeTag.codegen_policy) orelse CodegenPolicy{};
