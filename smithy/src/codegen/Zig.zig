...
pub const Expr = union(enum) {
    ...
    struct_literal: struct { identifier: []const u8, values: []const Expr },
    val_undefined,
    val_null,
    val_void,
    val_false,
    ...
    pub fn structLiteral(identifier: []const u8, values: []const Expr) Expr {
        return .{ .struct_literal = .{ .identifier = identifier, .values = values } };
    }
    ...
    test "structLiteral" {
        try testing.expectEqualDeep(
            Expr{ .struct_literal = .{
                .identifier = ".",
                .values = &.{ val(108), val("bar") },
            } },
            Expr.structLiteral(".", &.{ val(108), val("bar") }),
        );
    }
    ...
    pub fn format(self: Expr, comptime _: []const u8, _: fmt.FormatOptions, writer: anytype) !void {
        switch (self) {
            ...
            .struct_literal => |t| {
                try writer.print("{s}{{", .{t.identifier});
                for (t.values, 0..) |value, i| {
                    if (i > 0)
    ...
    test "format" {
        ...
        try testing.expectFmt(".{ 108, \"bar\" }", "{}", .{Expr{
            .struct_literal = .{
                .identifier = ".",
                .values = &.{ val(108), val("bar") },
            },
