...
pub const IssueBehavior = enum { skip, abort };
pub const AbortError = error.IssueAbortBehavior;
...
pub const IssuesBag = struct {
    allocator: Allocator,
    stats: Stats = .{},
    list: std.ArrayListUnmanaged(Issue) = .{},

    pub fn init(allocator: Allocator) IssuesBag {
        return .{ .allocator = allocator };
    }

    pub fn deinit(self: *IssuesBag) void {
    ...
    pub fn count(self: IssuesBag) usize {
        return self.list.items.len;
    }

    pub fn all(self: IssuesBag) []const Issue {
        return self.list.items;
    }

    pub fn add(self: *IssuesBag, issue: Issue) !void {
        switch (issue) {
...
test "IssuesBag" {
    var bag = IssuesBag.init(test_alloc);
    defer bag.deinit();
